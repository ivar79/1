...
const loginLimiter = rateLimit({
  ...
  max: 3, // strictly limit each IP to 3 requests per windowMs for admin login
  message: { success: false, error: "تعداد درخواست‌های ورود بیش از حد مجاز است. سیستم برای امنیت به مدت ۱۵ دقیقه قفل شد." }
});


import { OAuth2Client } from 'google-auth-library';
const rawClientId = process.env.GOOGLE_CLIENT_ID || process.env.VITE_GOOGLE_CLIENT_ID || "185697391194-bekne52lhc03q1tja6ac0megt72rgsgb.apps.googleusercontent.com";
const googleClient = new OAuth2Client(rawClientId.replace(/\s+/g, ""), process.env.GOOGLE_CLIENT_SECRET);
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production" && credential === "demo-google-token") {
      ...
      const ticket = await googleClient.verifyIdToken({
        idToken: credential,
        audience: rawClientId.replace(/\s+/g, ""),
      });
...
app.post("/api/auth/login", loginLimiter, async (req, res) => {
  ...
  try {
    ...
    if (found.length > 0) {
      ...
      isMatched = true;
      found.push({ id: "mock-admin-id", username: envAdminUser } as any);
    } else {
...
app.delete("/api/products/:id/hard", adminAuthMiddleware, async (req, res) => {
  ...
  try {
    ...
    console.log("CRASH ERROR:", err, (err as any).stack);
  ...
  try {
    ...
    if (showcaseOnly === "true") {
      filtered = filtered.filter(
        (p: any) => p.product.isFeatured && p.product.isActive,
      );
    }
...
app.get("/api/products/:id/reviews", async (req, res) => {
  try {
    ...
    console.error("GET REVIEWS ERROR", err); console.error("GET REVIEWS ERROR", (err as any).message); res.json([]); // Return empty array on DB failure
  }
});
...
app.get("/api/admin/reviews", adminAuthMiddleware, async (req, res) => {
  try {
    ...
    const reviews = await db.select({
      ...
      productTitle: schema.products.name,
  ...
  try {
    ...
    const customersList = Array.from(customerMap.values()).map((c) => {
      ...
      const referredOrders = allOrders.filter(
        (o: any) => o.customerMessage && o.customerMessage.includes(refCode),
      );
...
app.post("/api/admin/change-password", async (req, res) => {
  try {
    ...
    if (admin.length === 0) {
      // First-time DB initialization: verify with ENV fallback and INSERT to database
      if (currentPassword === (process.env.ADMIN_PASSWORD || "admin123")) {
        ...
        return res.json({ success: true, message: "رمز عبور جدید با موفقیت در دیتابیس ثبت شد. از این پس برای ورود فقط این رمز معتبر است." });
      }
      return res.status(404).json({ success: false, error: "رمز عبور فعلی (پیش‌فرض) اشتباه است" });
    }
...
app.get("/api/admin/commissions-report", adminAuthMiddleware, async (req, res) => {
  try {
    const db = getDb();
...
const DEFAULT_SETTINGS: Record<string, string> = {
  ...
  site_background: "",
  home_hero_badge: "پلتفرم تخصصی مقایسه و مشاوره مبلمان لوکس",
  home_hero_title: "مبلمان لوکس؛ خرید مطمئن، با شرایط اقساطی",
  home_hero_desc: "ما در انتخاب بهترین مبلمان همراه شما هستیم. مشاوره‌ی تخصصی، مقایسه‌ی برندهای معتبر و تضمین بهترین شرایط قیمتی برای خلق خانه‌ای رویایی.",
  footer_about: "خانه مبل؛ تجلی هنر و مهندسی در خلق فضاهای زیستی. ما مجموعه‌ای از برترین طراحی‌های مبلمان لوکس را برای شما گردآوری کرده‌ایم.",
};
...
app.get("/sitemap.xml", async (req, res) => {
  try {
    ...
    const baseUrl = "https://khane-moble.ir";
    ...
    for (const product of allProducts) {
      const date = new Date(product.updatedAt).toISOString().split('T')[0];
...
app.get("/api/blog", async (req, res) => {
  try {
    ...
    if (!isAdmin) {
      conditions = eq(schema.blogPosts.isPublished, true);
    }
...
app.get("/api/blog/:slug", async (req, res) => {
  try {
    ...
    const { slug } = req.params;
    const isAdmin = req.query.admin === "true";
    let conditions = eq(schema.blogPosts.slug, slug);
    if (!isAdmin) {
      conditions = and(eq(schema.blogPosts.slug, slug), eq(schema.blogPosts.isPublished, true)) as any;
    }
    ...
    if (!isAdmin) {
      await db.update(schema.blogPosts).set({ viewCount: sql`${schema.blogPosts.viewCount} + 1` }).where(eq(schema.blogPosts.id, post.id));
    }
...
app.post("/api/admin/blog", adminAuthMiddleware, async (req, res) => {
  try {
    const db = getDb();
    const { title, slug, content, excerpt, coverImage, isPublished, metaTitle, metaDescription } = req.body;
    ...
    return res.json({ success: true, data: newPost[0] });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
});

app.put("/api/admin/blog/:id", adminAuthMiddleware, async (req, res) => {
  try {
    const db = getDb();
    const id = String(req.params.id);
    const { title, slug, content, excerpt, coverImage, isPublished, metaTitle, metaDescription } = req.body;
    
    const existing = await db.query.blogPosts.findFirst({ where: eq(schema.blogPosts.id, id) });
    if (!existing) return res.status(404).json({ success: false, error: "Not found" });

    const publishedAt = (isPublished && !existing.isPublished) ? new Date() : existing.publishedAt;

    const updated = await db.update(schema.blogPosts).set({
      title,
      slug,
      content,
      excerpt,
...
app.delete("/api/admin/blog/:id", adminAuthMiddleware, async (req, res) => {
  try {
    const db = getDb();
    const id = String(req.params.id);
