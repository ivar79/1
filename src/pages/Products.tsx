                  ...
                  <div className="flex -space-x-2 space-x-reverse">
                    {compareList.map((c, i) => (
                      <div key={i} className="w-10 h-10 rounded-full border-2 border-stone-900 overflow-hidden bg-stone-800 shrink-0">
                        <img loading="lazy" src={c.product.images?.[0]} alt={c.product.name} className="w-full h-full object-cover" />
                      </div>
                    ))}
              ...
              <div className="flex-1 overflow-auto p-6">
                <div ref={scrollRef} className="flex md:grid gap-6 overflow-x-auto snap-x snap-mandatory pb-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']" style={{ gridTemplateColumns: `repeat(${compareList.length}, minmax(0, 1fr))` }}>
                  {compareList.map((item, idx) => (
                    <div key={idx} onClick={() => { navigate(`/product/${item.product.slug}`); setCompareModalOpen(false); }} className="cursor-pointer group flex flex-col space-y-6 min-w-[260px] md:min-w-0 snap-start h-full">
                      <div className="aspect-[4/3] rounded-2xl overflow-hidden bg-stone-100 border border-stone-200/50">
                        <img loading="lazy" src={item.product.images?.[0]} alt={item.product.name} className="w-full h-full object-cover" />
                      </div>
                      ...
                      <div className="space-y-3 pt-2">
                        ...
                        <div>
                          <p className="text-[10px] text-stone-400 mb-1 uppercase tracking-wider font-bold">تولیدکننده</p>
                          <p className="text-sm text-stone-800 font-medium bg-stone-50 p-3 rounded-xl border border-stone-100">
                            {item.showroomName}
                          </p>
                        </div>
                      ...
                      <div className="mt-auto pt-4">
                          ...
                          className="w-full inline-flex justify-center items-center pointer-events-none bg-stone-900 text-white text-xs font-bold py-3 px-4 rounded-xl hover:bg-stone-800 transition-colors"
                        >
                          مشاهده دقیق و مشاوره
                        </Link>
                      </div>
                    </div>
                  ))}
