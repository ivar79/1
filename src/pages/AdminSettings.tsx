...
import { Sofa, Phone, Mail, MapPin, Instagram, Send, MessageCircle, Save, CheckCircle, RefreshCw, Key, ShieldAlert, Image, LayoutTemplate, X, Upload, Paperclip } from "lucide-react";
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
                ...
                <label className="bg-stone-100 hover:bg-stone-200 text-stone-700 px-4 py-3 rounded-xl cursor-pointer text-xs font-bold transition-all whitespace-nowrap flex items-center shrink-0">
                  ...
                  <input type="file" accept="image/*" onChange={async (e) => {
                    if (e.target.files && e.target.files[0]) {
                      ...
                      const url = await uploadImage(file);
                      if (url) {
                        setSettings(prev => ({ ...prev, hero_images: url }));
                      } else {
              ...
              {settings.hero_images && (
                <div className="mt-3 relative w-32 h-20 rounded-lg overflow-hidden border border-stone-200 shadow-sm">
                  <img src={settings.hero_images} alt="Background Preview" className="w-full h-full object-cover" />
                  <button 
                    type="button" 
                    onClick={() => setSettings(prev => ({ ...prev, hero_images: "" }))}
        ...
        <div className="bg-white p-6 rounded-2xl border border-stone-200/60 shadow-sm space-y-5">
          <h3 className="text-sm font-bold text-stone-900 border-b border-stone-100 pb-3 flex items-center gap-2">
            ...
            <span>تنظیمات صفحه اصلی و فوتر</span>
          </h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-700">تیتر نشان صفحه اصلی</label>
              <input type="text" name="home_hero_badge" value={settings.home_hero_badge || ""} onChange={handleChange} className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2 text-sm text-right outline-none focus:border-stone-400 transition-colors" dir="rtl" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-bold text-stone-700">تیتر اصلی صفحه اصلی</label>
              <input type="text" name="home_hero_title" value={settings.home_hero_title || ""} onChange={handleChange} className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-2 text-sm text-right outline-none focus:border-stone-400 transition-colors" dir="rtl" />
            </div>
            <div className="space-y-1 md:col-span-2">
              <label className="text-xs font-bold text-stone-700">توضیحات صفحه اصلی</label>
              <textarea name="home_hero_desc" value={settings.home_hero_desc || ""} onChange={handleChange} className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-right outline-none focus:border-stone-400 transition-colors min-h-[80px]" dir="rtl" />
            </div>
            <div className="space-y-1 md:col-span-2">
              <label className="text-xs font-bold text-stone-700">متن درباره ما در فوتر</label>
              <textarea name="footer_about" value={settings.footer_about || ""} onChange={handleChange} className="w-full bg-stone-50 border border-stone-200 rounded-xl px-4 py-3 text-sm text-right outline-none focus:border-stone-400 transition-colors min-h-[80px]" dir="rtl" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-stone-200/60 shadow-sm space-y-5">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-stone-500 block">تصویر بنر درباره ما</label>
              <div className="relative flex items-center">
                <input
                  ...
                  onChange={handleChange}
                  className="w-full text-xs font-medium border border-stone-200 p-3.5 pr-14 rounded-full bg-stone-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/30 transition-all text-left"
                  dir="ltr"
                  placeholder="https://..."
                />
                <label className="absolute right-1.5 top-1.5 bottom-1.5 w-10 bg-blue-500 hover:bg-blue-600 text-white rounded-full cursor-pointer flex items-center justify-center transition-all shadow-md shadow-blue-500/20" title="آپلود تصویر (مانند تلگرام)">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      const file = e.target.files?.[0];
                      if (!file) return;
                      setStatus({ type: "success", message: "در حال آپلود عکس بنر در فضای ابری..." });
                      const url = await uploadImage(file);
                      if (url) {
                        setSettings((prev: any) => ({ ...prev, about_banner_image: url }));
                        setStatus({ type: "success", message: "عکس با موفقیت در فضای ابری آپلود شد." });
                      } else {
                        setStatus({ type: "error", message: "خطا در آپلود عکس" });
                        setTimeout(() => setStatus(null), 4000);
                      }
                    }}
                    className="hidden"
              ...
              <p className="text-[10px] text-stone-400 mt-1">از طریق دکمه بالا عکس را آپلود کنید تا مستقیماً در باکت لیارا ذخیره شود.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-stone-100 mt-2">
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-stone-500 block">ویژگی اول: عنوان</label>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-medium border border-stone-200 p-3 rounded-xl bg-stone-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-stone-950/25 transition-all text-right"
                />
                <label className="text-[11px] font-bold text-stone-500 block mt-2">توضیحات ویژگی اول</label>
                <textarea
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-stone-500 block">ویژگی دوم: عنوان</label>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-medium border border-stone-200 p-3 rounded-xl bg-stone-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-stone-950/25 transition-all text-right"
                />
                <label className="text-[11px] font-bold text-stone-500 block mt-2">توضیحات ویژگی دوم</label>
                <textarea
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-stone-500 block">باکس ارزش اول: عنوان</label>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-medium border border-stone-200 p-3 rounded-xl bg-stone-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-stone-950/25 transition-all text-right"
                />
                <label className="text-[11px] font-bold text-stone-500 block mt-2">توضیحات باکس ارزش اول</label>
                <textarea
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-bold text-stone-500 block">باکس ارزش دوم: عنوان</label>
                <input
                  type="text"
                  ...
                  className="w-full text-xs font-medium border border-stone-200 p-3 rounded-xl bg-stone-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-stone-950/25 transition-all text-right"
                />
                <label className="text-[11px] font-bold text-stone-500 block mt-2">توضیحات باکس ارزش دوم</label>
                <textarea
