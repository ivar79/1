...
import { Plus, Edit2, Check, X, Sofa, Trash2, Save, Sparkles, Image, CheckSquare , Eye, EyeOff } from "lucide-react";
...
export default function AdminProducts() {
  ...
  const handleDeleteAndBackup = async (item: any) => {
    ...
    try {
      ...
      const backupRes = await fetch(`/api/products/${item.id}/backup`, {
      ...
      if (backupRes.ok) {
        ...
        if (backupData.success) {
          ...
          a.href = url;
          a.download = `backup-comprehensive-product-${item.id}.csv`;
      ...
      const delPath = `/api/products/${item.id}/hard`;
