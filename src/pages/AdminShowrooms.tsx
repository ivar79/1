...
import { Plus, Edit2, Check, X, Store, Phone, Percent, MapPin, Save, ShieldAlert, CircleDot , Trash2 } from "lucide-react";
...
export default function AdminShowrooms() {
  ...
  const handleDeleteAndBackup = async (item: any) => {
    ...
    try {
      ...
      const backupRes = await fetch(`/api/showrooms/${item.id}/backup`, {
      ...
      if (backupRes.ok) {
        ...
        if (backupData.success) {
          ...
          a.href = url;
          a.download = `backup-comprehensive-showroom-${item.id}.csv`;
      ...
      const delPath = `/api/showrooms/${item.id}/hard`;
