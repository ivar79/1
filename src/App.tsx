...
import { LayoutDashboard, FileText, ShoppingCart, Sofa, Store, Receipt, LogOut, ChevronLeft, Menu, Settings, Users , MessageSquare} from "lucide-react";
...
function MainLayout() {
  ...
  const isLoginPage = location.pathname === "/admin/portal-entrance-secure";
    ...
    <div className="bg-stone-50 min-h-screen text-stone-900 font-sans selection:bg-stone-900 selection:text-stone-50 flex flex-col justify-between">
      ...
      <main className="flex-grow pb-20 lg:pb-0">
        <Routes>
          ...
          <Route path="/admin/portal-entrance-secure" element={<AdminLogin />} />
...
function AdminRequiredGuard({ children }: { children: React.ReactNode }) {
  ...
  if (!token) {
    return <Navigate to="/admin/portal-entrance-secure" replace />;
  }
  return <>{children}</>;
}
...
function AdminWorkspaceLayout() {
  ...
  const handleLogout = () => {
    ...
    navigate("/admin/portal-entrance-secure");
  };
  ...
  const menuItems = [
    ...
    { path: "/admin/blog", name: "بلاگ و مقالات", icon: <FileText className="w-4 h-4" /> },
